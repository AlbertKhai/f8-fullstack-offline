...
export default function ModalShare({ isOpen, onOpen }) {
  ...
  const mindmap = useSelector(({ mindmap }) => mindmap.listMindmap.find(({ id }) => id === idMindmap))
  const { data, isLoading, error } = mindmapShare.useGetMindmapShareQuery(idMindmap)
  const [postMindmapShare, resultPost] = mindmapShare.usePostMindmapShareMutation()
  ...
  const handleShare = async (e) => {
    ...
    const dataShare = { ...mindmap, title: formData.get('title'), desc: formData.get('desc'), image: formData.get('image') }
  ...
  return (
    <Transition.Root show={isOpen} as={Fragment}>
      <Dialog as='div' className='relative z-10' initialFocus={cancelButtonRef} onClose={onOpen}>
        <Transition.Child
          as={Fragment}
              ...
              <Dialog.Panel className='relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg'>
                <div className='relative bg-white rounded-lg shadow dark:bg-gray-700 sm:items-start'>
                  ...
                  <div className='flex items-center justify-center gap-3 p-4 md:p-5 border-b rounded-t dark:border-gray-600'>
                    ...
                    <BtnSwitch isShare={isShare} onShare={setIsShare} />
                  </div>
                  {/* Modal body */}
                  <form ref={formRef} id='formShareMindmap' onSubmit={handleShare} className='p-4 md:p-5'>
                    <div className='relative grid gap-4 mb-4 grid-cols-2'>
                      <div className='col-span-2'>
                        ...
                        <input
                          ...
                          defaultValue={dataMindmap.title}
                          required
                        />
                      </div>
                      <div className='col-span-2'>
                      ...
                      <div className='col-span-2'>
                        <button
                          ...
                          class='bg-gray-300 hover:bg-gray-400 text-base text-gray-800 font-bold py-2 px-4 mx-auto gap-2 rounded flex items-center'
                        >
